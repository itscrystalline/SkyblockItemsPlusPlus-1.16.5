...
public class Inactive_Campfire_Initiate_Badge extends AccessoryItem {
    public Inactive_Campfire_Initiate_Badge(Properties properties) {
        super(properties);
    }

    @Override
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> text, ITooltipFlag toolTip) {
        text.add(new StringTextComponent(ColorText.GRAY + "Have a 1 in 50 chance to heal 1 health every second"));
        ...
        text.add(new StringTextComponent(ColorText.GRAY.toString() + ColorText.BOLD + "UNCOMMON ACCESSORY"));
        text.add(new StringTextComponent(""));
        text.add(new StringTextComponent(""));
        text.add(new StringTextComponent(ColorText.GOLD.toString() + "RIGHT CLICK to activate this accessory! (5 XP)"));
    ...
    public ActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        if (player.experienceLevel >= 5)
        {
            ...
            player.inventory.add(ItemInit.CAMPFIRE_INITIATE_BADGE.get().getDefaultInstance());
