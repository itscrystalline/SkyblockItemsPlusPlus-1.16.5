...
public class Inactive_Campfire_God_Badge extends AccessoryItem {
    public Inactive_Campfire_God_Badge(Properties properties) {
        super(properties);
    }

    @Override
    public void appendHoverText(ItemStack stack, @Nullable World world, List<ITextComponent> text, ITooltipFlag toolTip) {
        text.add(new StringTextComponent(ColorText.GRAY + "Have a 1 in 20 chance to heal 1 health every second"));
        text.add(new StringTextComponent(ColorText.GRAY + "after 2 seconds while burning."));
        text.add(new StringTextComponent(ColorText.GRAY + "Gain permant Regeneration 1 and Resistance 1."));
        text.add(new StringTextComponent(""));
        text.add(new StringTextComponent(ColorText.GRAY + "This item can be reforged!"));
        text.add(new StringTextComponent(ColorText.GRAY.toString() + ColorText.BOLD.toString() + "LEGENDARY ACCESSORY"));
        text.add(new StringTextComponent(""));
        text.add(new StringTextComponent(""));
        text.add(new StringTextComponent(ColorText.GOLD.toString() + "RIGHT CLICK to activate this accessory! (40 XP)"));
    ...
    public ActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        if (player.experienceLevel >= 40)
        {
            ...
            player.inventory.add(ItemInit.CAMPFIRE_GOD_BADGE.get().getDefaultInstance());
